...
class TestKnownPrefixes:
    def test_openai_sk_key(self):
        text = "Using key [REDACTED_API_KEY]"
    ...
    def test_github_pat_classic(self):
        result = redact_sensitive_text("token: [REDACTED_GITHUB_TOKEN]")
        assert "abc123def456" not in result
...
class TestEnvAssignments:
    def test_export_api_key(self):
        text = "export OPENAI_API_KEY=[REDACTED_API_KEY]"
        result = redact_sensitive_text(text)
...
class TestAuthHeaders:
    def test_bearer_token(self):
        text = "Authorization: Bearer [REDACTED_API_KEY]"
        result = redact_sensitive_text(text)
...
class TestPassthrough:
    ...
    def test_non_string_input_dict_coerced_and_redacted(self):
        result = redact_sensitive_text({"token": "[REDACTED_API_KEY]"})
        assert "abc123def456" not in result
...
class TestSecretCapturePayloadRedaction:
    ...
    def test_raw_secret_field_redacted(self):
        text = '{"raw_secret": "[REDACTED_GITHUB_TOKEN]"}'
        result = redact_sensitive_text(text)
        assert "abc123def456" not in result
