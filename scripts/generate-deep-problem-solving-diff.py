#!/usr/bin/env python3
"""Generate deep problem-solving behavior parity/superiority proof artifacts."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cases",
        default="docs/parity/deep-problem-solving-cases.json",
        type=Path,
        help="Deep problem-solving case manifest",
    )
    parser.add_argument(
        "--out-json",
    ...
    parser.add_argument(
        "--out-md",
        default="docs/parity/deep-problem-solving-diff.md",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=8,
        type=int,
        help="Minimum strictly-superior cases required for the deep problem-solving gate",
    )
    parser.add_argument(
        "--min-total-cases",
        default=10,
        type=int,
        help="Minimum deep problem-solving cases required for the gate",
    )
    parser.add_argument("--check", action="store_true", help="Fail when deep problem-solving gate fails")
...
def validate_case(
    ...
    for field in ["domain", "upstream_behavior", "ultra_behavior", "why"]:
...
def render_md(payload: dict[str, Any]) -> str:
    ...
    lines: list[str] = [
        ...
        f"- Deep problem-solving ratio: `{summary['deep_problem_solving_ratio']}`",
        f"- Superior cases: `{summary['superior_cases']}`",
        f"- Regressions: `{summary['regressions']}`",
        f"- Gaps: `{summary['gaps']}`",
        f"- Unverified cases: `{summary['unverified_cases']}`",
        f"- Missing Rust test refs: `{summary['missing_rust_test_refs']}`",
        "",
        "## Summary",
        "",
        "| Metric | Value |",
    ...
    lines.extend(["", "## Cases", "", "| Case | Domain | Classification | Score |", "| --- | --- | --- | ---: |"])
...
def main() -> int:
    ...
    manifest = load_json((repo_root / args.cases).resolve())
    ...
    ratio = round(equal_or_better / total, 4) if total else 0.0
    ...
    if total < args.min_total_cases:
        gate_failures.append(
            {
                "kind": "total_cases",
                "message": "deep problem-solving case count is below threshold",
                "actual": total,
                "limit": args.min_total_cases,
            }
        )
    if ratio < 1.0:
        gate_failures.append(
            {
                "kind": "deep_problem_solving_ratio",
                "message": "deep problem-solving ratio is below 1.0",
                "actual": ratio,
                "limit": 1.0,
            }
        )
        ...
        gate_failures.append(
            {
                ...
                "message": "strictly superior deep problem-solving cases are below threshold",
                "actual": superior,
                "limit": args.min_superiority_cases,
            }
        )
    if regressions:
        gate_failures.append({"kind": "regressions", "message": "deep problem-solving regressions are present", "actual": regressions, "limit": 0})
    if gaps:
        gate_failures.append({"kind": "gaps", "message": "deep problem-solving gaps are present", "actual": gaps, "limit": 0})
    if unverified:
        gate_failures.append({"kind": "unverified", "message": "unverified deep problem-solving cases are present", "actual": unverified, "limit": 0})
    if missing_rust_refs:
        gate_failures.append(
            {
                "kind": "missing_rust_test_refs",
                "message": "deep problem-solving cases reference missing Rust tests",
        ...
        gate_failures.append(
            {
                ...
                "message": "deep problem-solving cases reference missing source artifacts",
    ...
    summary = {
        ...
        "deep_problem_solving_ratio": ratio,
