...
print_config_summary() {
  ...
  echo "  summary sink order: ${UPSTREAM_SYNC_SUMMARY_SINK_ORDER:-contextlattice,github,local}"
  echo "  summary context project: ${UPSTREAM_SYNC_SUMMARY_CONTEXT_PROJECT:-<repo-name>}"
  echo "  summary topic path: ${UPSTREAM_SYNC_SUMMARY_CONTEXT_TOPIC_PATH:-ops/upstream-sync}"
  echo "  summary file key: ${UPSTREAM_SYNC_SUMMARY_CONTEXT_FILE_NAME:-ops/upstream-sync.md}"
  echo "  summary fallback issue: ${UPSTREAM_SYNC_SUMMARY_GITHUB_ISSUE:-13}"
  echo "  summary fallback log: ${UPSTREAM_SYNC_SUMMARY_LOCAL_PATH:-<repo>/.sync-reports/upstream-sync-summary-fallback.log}"
