...
use clap::Parser;
    ...
    fn cli_parse_config_dir() {
        let cli = Cli::try_parse_from(["hermes", "-C", "/tmp/hermes"]).unwrap();
        assert_eq!(cli.config_dir.as_deref(), Some("/tmp/hermes"));
    }

    #[test]
    fn cli_parse_model_flag() {
        let cli = Cli::try_parse_from(["hermes", "-m", "claude-3-opus"]).unwrap();
    ...
    fn cli_parse_ignore_flags() {
        let cli =
            Cli::try_parse_from(["hermes", "--ignore-user-config", "--ignore-rules"]).unwrap();
        assert!(cli.ignore_user_config);
        assert!(cli.ignore_rules);
    }
    ...
    #[test]
    fn cli_parse_doctor() {
        let cli = Cli::try_parse_from(["hermes", "doctor"]).unwrap();
        assert!(matches!(
            cli.command,
    ...
    fn cli_parse_update_with_check_flag() {
        let cli = Cli::try_parse_from(["hermes", "update", "--check"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_rotate_provenance_key() {
        let cli = Cli::try_parse_from(["hermes", "rotate-provenance-key", "--json"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_route_learning_reset() {
        let cli = Cli::try_parse_from(["hermes", "route-learning", "reset", "--json"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_route_health_show() {
        let cli = Cli::try_parse_from(["hermes", "route-health", "show", "--json"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_resume_latest_default() {
        let cli = Cli::try_parse_from(["hermes", "resume"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_resume_specific_session() {
        let cli = Cli::try_parse_from(["hermes", "resume", "abc123"]).unwrap();
        match cli.command {
            Some(CliCommand::Resume { session_id }) => {
    ...
    fn cli_parse_elite_check() {
        let cli = Cli::try_parse_from(["hermes", "elite-check", "--json", "--strict"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_logs_default() {
        let cli = Cli::try_parse_from(["hermes", "logs"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_logs_with_count() {
        let cli = Cli::try_parse_from(["hermes", "logs", "50"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_profile() {
        let cli = Cli::try_parse_from(["hermes", "profile", "list"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_profile_create() {
        let cli = Cli::try_parse_from(["hermes", "profile", "create", "work"]).unwrap();
        match cli.command {
    ...
    fn cli_parse_config_set() {
        let cli = Cli::try_parse_from(["hermes", "config", "set", "model", "gpt-4o"]).unwrap();
        match cli.command {
