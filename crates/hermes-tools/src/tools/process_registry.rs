...
impl ToolHandler for ProcessRegistryHandler {
        ...
        match action {
            "register" => {
                ...
                if name.is_empty() || pid <= 0 {
                    return Err(ToolError::InvalidParams("register requires name + pid".into()));
                }
                self.entries.lock().map_err(|e| ToolError::ExecutionFailed(e.to_string()))?.insert(name.to_string(), pid);
                Ok(json!({"status":"registered","name":name,"pid":pid}).to_string())
            }
            _ => {
                let entries = self.entries.lock().map_err(|e| ToolError::ExecutionFailed(e.to_string()))?;
    ...
    fn schema(&self) -> ToolSchema {
        ...
        props.insert("action".into(), json!({"type":"string","enum":["list","register"]}));
        props.insert("name".into(), json!({"type":"string"}));
        props.insert("pid".into(), json!({"type":"integer"}));
        tool_schema("process_registry", "Register/list background process metadata.", JsonSchema::object(props, vec![]))
    }
}
