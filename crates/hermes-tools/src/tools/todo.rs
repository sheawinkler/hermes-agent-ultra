...
impl ToolHandler for TodoHandler {
    async fn execute(&self, params: Value) -> Result<String, ToolError> {
        ...
        match action {
            ...
            "update" => {
                ...
                let priority = params.get("priority").and_then(|v| v.as_str());
                self.backend.update(id, title, description, status, priority).await
            }
            "list" => {
                let status = params.get("status").and_then(|v| v.as_str());
            ...
            other => Err(ToolError::InvalidParams(format!(
                "Unknown action: '{}'. Use 'create', 'update', or 'list'.", other
            ))),
        }
    }
    ...
    fn schema(&self) -> ToolSchema {
        ...
        props.insert("description".into(), json!({
            "type": "string",
        ...
        props.insert("priority".into(), json!({
            "type": "string",
...
mod tests {
    ...
    impl TodoBackend for MockTodoBackend {
        async fn create(&self, title: &str, _description: Option<&str>, _priority: Option<&str>) -> Result<String, ToolError> {
            Ok(format!("Created todo: {}", title))
        }
        async fn update(&self, id: &str, _title: Option<&str>, _description: Option<&str>, _status: Option<&str>, _priority: Option<&str>) -> Result<String, ToolError> {
    ...
    async fn test_todo_create() {
        ...
        let result = handler.execute(json!({"action": "create", "title": "Test task"})).await.unwrap();
