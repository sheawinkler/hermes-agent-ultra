...
impl CdpBrowserBackend {
    ...
    async fn cdp_command(&self, method: &str, params: Value) -> Result<Value, ToolError> {
        ...
        let targets_resp = self.client
            .get(format!("{}/json", self.endpoint))
            .send()
            .await
            .map_err(|e| ToolError::ExecutionFailed(format!(
                "Failed to connect to Chrome CDP at {}: {}", self.endpoint, e
            )))?;

        let targets: Vec<Value> = targets_resp.json().await
            .map_err(|e| ToolError::ExecutionFailed(format!("Failed to parse CDP targets: {}", e)))?;
...
impl BrowserBackend for CdpBrowserBackend {
    async fn navigate(&self, url: &str) -> Result<String, ToolError> {
        let result = self.cdp_command("Page.navigate", json!({"url": url})).await?;
        Ok(json!({"status": "navigated", "url": url, "cdp": result}).to_string())
    }

    async fn snapshot(&self) -> Result<String, ToolError> {
        let result = self.cdp_command("Accessibility.getFullAXTree", json!({})).await?;
    ...
    async fn click(&self, selector: &str) -> Result<String, ToolError> {
        ...
        let result = self.cdp_command("Runtime.evaluate", json!({"expression": js})).await?;
    ...
    async fn r#type(&self, selector: &str, text: &str) -> Result<String, ToolError> {
        ...
        let result = self.cdp_command("Runtime.evaluate", json!({"expression": js})).await?;
        Ok(json!({"status": "typed", "selector": selector, "text": text, "cdp": result}).to_string())
    }
    ...
    async fn scroll(&self, direction: &str, amount: Option<u32>) -> Result<String, ToolError> {
        ...
        let result = self.cdp_command("Runtime.evaluate", json!({"expression": js})).await?;
        Ok(json!({"status": "scrolled", "direction": direction, "amount": px, "cdp": result}).to_string())
    }

    async fn go_back(&self) -> Result<String, ToolError> {
        let result = self.cdp_command("Runtime.evaluate", json!({"expression": "history.back(); 'back'"})).await?;
        Ok(json!({"status": "navigated_back", "cdp": result}).to_string())
    }

    async fn press(&self, key: &str) -> Result<String, ToolError> {
        let result = self.cdp_command("Input.dispatchKeyEvent", json!({
            "type": "keyDown",
            "key": key,
        })).await?;
    ...
    async fn get_images(&self, selector: Option<&str>) -> Result<String, ToolError> {
        ...
        let result = self.cdp_command("Runtime.evaluate", json!({"expression": js, "returnByValue": true})).await?;
    ...
    async fn vision(&self, instruction: &str) -> Result<String, ToolError> {
        ...
        let result = self.cdp_command("Page.captureScreenshot", json!({"format": "png"})).await?;
        Ok(json!({
    ...
    async fn console(&self, action: &str) -> Result<String, ToolError> {
        match action {
            ...
            "clear" => {
                let result = self.cdp_command("Runtime.evaluate", json!({"expression": "console.clear(); 'cleared'"})).await?;
