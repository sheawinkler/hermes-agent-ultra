...
impl SqliteSessionSearchBackend {
    ...
    pub fn new(db_path: &str) -> Result<Self, ToolError> {
        let conn = Connection::open(db_path)
            .map_err(|e| ToolError::ExecutionFailed(format!("Failed to open sessions DB: {}", e)))?;
    ...
    pub fn index_message(
        ...
        let conn = self.conn.lock().map_err(|e| ToolError::ExecutionFailed(e.to_string()))?;
        conn.execute(
    ...
    async fn search(&self, query: &str, limit: usize) -> Result<String, ToolError> {
        ...
        let mut stmt = conn.prepare(
            "SELECT session_id, role, content, timestamp, rank
             ...
             ORDER BY rank
             LIMIT ?2"
        ).map_err(|e| ToolError::ExecutionFailed(format!("Failed to prepare search query: {}", e)))?;
