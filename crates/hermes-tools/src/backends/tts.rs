...
impl MultiTtsBackend {
    ...
    async fn edge_tts(&self, text: &str, voice: &str) -> Result<String, ToolError> {
        ...
        let output_path = std::env::temp_dir()
            .join(format!("hermes_tts_{}.mp3", uuid::Uuid::new_v4()));

        let output = tokio::process::Command::new("edge-tts")
            .arg("--voice").arg(voice)
            .arg("--text").arg(text)
            .arg("--write-media").arg(output_path.to_str().unwrap_or("output.mp3"))
            .output()
            .await
            .map_err(|e| ToolError::ExecutionFailed(format!(
                "edge-tts command failed (is it installed? pip install edge-tts): {}", e
            )))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(ToolError::ExecutionFailed(format!("edge-tts error: {}", stderr)));
        }
    ...
    async fn elevenlabs_tts(&self, text: &str, voice: &str) -> Result<String, ToolError> {
        let api_key = self.elevenlabs_key.as_ref()
        ...
        if !status.is_success() {
            ...
            return Err(ToolError::ExecutionFailed(format!("ElevenLabs error ({}): {}", status, text)));
        }
    ...
    async fn openai_tts(&self, text: &str, voice: &str) -> Result<String, ToolError> {
        ...
        if !status.is_success() {
            let text = resp.text().await.unwrap_or_default();
            return Err(ToolError::ExecutionFailed(format!("OpenAI TTS error ({}): {}", status, text)));
        }
    ...
    async fn synthesize(&self, text: &str, voice: Option<&str>, provider: Option<&str>) -> Result<String, ToolError> {
        ...
        match provider {
            ...
            other => Err(ToolError::InvalidParams(format!(
                "Unknown TTS provider: '{}'. Use 'edge_tts', 'elevenlabs', or 'openai'.", other
            ))),
        }
    }
}
