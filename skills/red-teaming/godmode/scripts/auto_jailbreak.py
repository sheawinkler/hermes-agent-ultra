...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.hermes-agent-ultra-agent-ultra")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HERMES_HOME", Path.home() / ".hermes-agent-ultra")) / "skills" / "red-teaming" / "godmode"
...
HERMES_HOME = Path(os.getenv("HERMES_HOME", Path.home() / ".hermes-agent-ultra"))
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.hermes-agent-ultra/prefill.json."""
