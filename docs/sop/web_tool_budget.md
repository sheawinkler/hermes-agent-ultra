...
1. **Pre-check** (`apply_web_tool_budget`): allow if the tool’s pool `used < max` and aggregate backstop (if set) not exceeded.
...
3. **URL dedup** (`apply_web_url_dedup`): before budget, block `web_extract` / `browser_navigate` when the same normalized URL already has a **successful** `web_extract` in `ctx` messages.
